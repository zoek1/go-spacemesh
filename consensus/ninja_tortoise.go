...
func (ni *ninjaTortoise) forBlockInView(blocks []mesh.BlockID, layer mesh.LayerID, foo func(*ninjaBlock)) int {
	stack := list.New()
	count := -1 * len(blocks) //dont count pattern blocks
...
func (ni *ninjaTortoise) findMinimalGoodLayer(i mesh.LayerID, b []*ninjaBlock) mesh.LayerID {
	...
	fmt.Println("found minimal layer ", l)
	return l
}
...
func (ni *ninjaTortoise) UpdateTables(B []*mesh.Block, i mesh.LayerID) mesh.LayerID { //i most recent layer
	...
	for j := l; j < i; j++ {
		...
		ni.tComplete[p] = flag
		if flag {
			fmt.Println("found new complete and good layer ", l)
			ni.pBase = &p
		}
	}
