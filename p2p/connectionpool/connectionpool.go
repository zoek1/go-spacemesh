...
type networker interface {
	Dial(address string, remotePublicKey cryptoSign.PublicKey) (net.Connection, error) // Connect to a remote node. Can send when no error.
...
func NewConnectionPool(network networker, lPub cryptoSign.PublicKey) *ConnectionPool {
...
func (cp *ConnectionPool) handleDialResult(rPub cryptoSign.PublicKey, result dialResult) {
	cp.pendMutex.Lock()
...
func (cp *ConnectionPool) handleNewConnection(rPub cryptoSign.PublicKey, newConn net.Connection, source net.ConnectionSource) {
	cp.connMutex.Lock()
...
func (cp *ConnectionPool) GetConnection(address string, remotePub cryptoSign.PublicKey) (net.Connection, error) {
	cp.connMutex.RLock()
	if cp.shutdown {
...
func (cp *ConnectionPool) GetConnectionIfExists(remotePub cryptoSign.PublicKey) (net.Connection, error) {
	cp.connMutex.RLock()
	if cp.shutdown {
		cp.connMutex.RUnlock()
		return nil, errors.New("ConnectionPool was shut down")
