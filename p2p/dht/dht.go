...
type DHT interface {
	...
	Lookup(pubkey cryptoSign.PublicKey) (node.Node, error)
...
func (d *KadDHT) Lookup(pubkey cryptoSign.PublicKey) (node.Node, error) {
	dhtid := pubkey.Bytes()
...
func (d *KadDHT) kadLookup(id cryptoSign.PublicKey, searchList []node.Node) (node.Node, error) {
	...
	for {
		...
		if len(res) > 0 {
			...
			searchList = node.SortByDhtID(res, id.Bytes())
		}
...
func (d *KadDHT) findNodeOp(servers []node.Node, queried map[string]struct{}, id cryptoSign.PublicKey,
	...
	for i := 0; i < l; i++ {
		...
		go func(server node.Node, id cryptoSign.PublicKey) {
