...
func (p *findNodeProtocol) sendRequestMessage(server cryptoSign.PublicKey, payload []byte, reqID crypto.UUID, responseChan chan findNodeResults) (bool, error) {
...
func (p *findNodeProtocol) sendResponseMessage(server cryptoSign.PublicKey, reqID, payload []byte) error {
	findnode := &pb.FindNode{}
	findnode.Req = false
	findnode.ReqID = reqID
	findnode.Payload = payload
...
func (p *findNodeProtocol) FindNode(serverNode node.Node, target cryptoSign.PublicKey) ([]node.Node, error) {
...
func (p *findNodeProtocol) handleIncomingRequest(sender cryptoSign.PublicKey, reqID, msg []byte) {
	req := &pb.FindNodeReq{}
...
func fromNodeInfos(nodes []*pb.NodeInfo) []node.Node {
	...
	for i, n := range nodes {
		pubk, err := cryptoSign.NewPublicKey(n.NodeId)
		if err != nil {
