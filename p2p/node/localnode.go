...
func (n *LocalNode) PrivateKey() cryptoSign.PrivateKey {
	return n.privKey
}
...
func newLocalNodeWithKeys(pubKey cryptoSign.PublicKey, privKey cryptoSign.PrivateKey, address string, networkID int8, persist bool) (*LocalNode, error) {
...
func newLocalNodeFromFile(address string, d *nodeFileData, persist bool) (*LocalNode, error) {
	...
	priv, err := cryptoSign.NewPrivateKeyFromString(d.PrivKey)
	if err != nil {
		return nil, err
	}

	pub, err := cryptoSign.NewPublicKeyFromString(d.PubKey)
	if err != nil {
		return nil, err
	}
